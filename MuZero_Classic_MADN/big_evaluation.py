...
from MuZero_Classic_MADN.evaluate_agent_stochastic import evaluate_agent_parallel, fairness_check
...
# ---------------------------------------------------------------------------
TRAINED_AGENTS = [
    f"{FOLDER}TEAMstochastic_muzero_madn_params_lr0.005_g1500_it100_seed30.pkl",
    f"{FOLDER}TEAMstochastic_muzero_madn_params_lr0.005_g1500_it100_seed32.pkl",
    f"{FOLDER}TEAMstochastic_muzero_madn_params_lr0.005_g1500_it100_seed34.pkl",
    f"{FOLDER}TEAMstochastic_muzero_madn_params_lr0.005_g1500_it100_seed36.pkl",
]
...
def run_matchup(spec_a, spec_b, batch_size=BATCH_SIZE):
    """
    ...
    return evaluate_agent_parallel(params_a, params_b, params_a, params_b,batch_size=batch_size)
...
def run_big_evaluation(
    ...
    for agent_spec in trained_agents:
        for baseline_spec in baseline_agents:
            name_a = short_name(agent_spec)
            name_b = short_name(baseline_spec)

            print(f"\n[{name_a}]  vs  [{name_b}]")
            t0 = time()
            result = run_matchup(agent_spec, baseline_spec, batch_size=batch_size)
            elapsed = time() - t0
    ...
    for i, j in pairs:
        ...
        print(f"\n[{name_a}]  vs  [{name_b}]")
        t0 = time()
        result = run_matchup(spec_a, spec_b, batch_size=batch_size)
        elapsed = time() - t0
        print(f"  → done in {elapsed:.1f}s")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Evaluating agents for TEAM stochastic MADN...")
        run_big_evaluation()
    finally:
        wandb.finish()