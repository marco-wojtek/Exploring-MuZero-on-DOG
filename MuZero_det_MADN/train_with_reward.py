...
def test_training(config, params=None, opt_state=None):
    ...
    for it in range(iterations):
        ...
        if ((it+1) % 50 == 0) or (it == iterations - 1):
            ...
            with open(f'MuZero_det_MADN/models/opt_state/TEAMmuzero_madn_opt_state_lr{config["learning_rate"]}_g{config["num_games_per_iteration"]}_it{it+1}_seed{config["seed"]}.pkl', 'wb') as f:
                pickle.dump(opt_state, f)
