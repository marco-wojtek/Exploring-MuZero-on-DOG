...
class RepresentationNetwork2(nn.Module):
    ...
    def __call__(self, x):
        ...
        # Channel-Dimension hinzufügen für Conv2D
...
class DynamicsNetwork4(nn.Module):
    ...
    def __call__(self, latent_state, action):
        ...
        # Konditionierung: Action moduliert den Latent State
        ...
        # Residual Connection zum ORIGINAL Latent State
        x = nn.Dense(self.latent_dim)(x)
...
def recurrent_inference_fn(params, rng_key, action, embedding):
    ...
    reward = jnp.sum(reward_probs * support, axis=-1)  
    
    discount_probs = jax.nn.softmax(discount_logits, axis=-1)
    discount = jnp.sum(discount_probs * support, axis=-1)  
