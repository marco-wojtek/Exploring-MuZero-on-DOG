...
def combined_recurrent_fn(params, rng_key, action, embedding):
    ...
    # Sample one chance outcome per batch element; different simulations will
    # draw different outcomes, keeping the PUCT tree broad rather than a single chain.
    chance_outcome = jax.random.categorical(rng_key, chance_logits)  # [B]
...
def run_muzero_mcts_fast(params, rng_key, observations, invalid_actions,
    ...
    policy_output = mctx.gumbel_muzero_policy(
        ...
        recurrent_fn=combined_recurrent_fn, # Funktion für Schritte im latenten Raum
        num_simulations=num_simulations,
        max_depth=max_depth,
        invalid_actions=invalid_actions,
